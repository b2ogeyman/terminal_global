...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def starter_strategy(self, game_state):
        ...
        if game_state.turn_number <= 15:
            self.attack_monte_carlo(game_state, DEMOLISHER, score_th=500)
        #elif game_state.turn_number <= 40 and game_state.turn_number % 2 == 0:
        #    self.attack_monte_carlo(game_state, DEMOLISHER)
    ...
    def attack_monte_carlo(self, game_state, unit_type, score_th = None):
        ...
        for deploy_location in deploy_locations:
            max_under_budget = game_state.number_affordable(unit_type)
            ...
            attack_all = tmp_game_state.attack_score(deploy_location, max_under_budget, unit_type)
            attack_score = attack_all["score"]
            if attack_score > best_score:
                best_score = attack_score
                best_location = deploy_location
                best_number = max_under_budget
                best_all = attack_all
                # gamelib.debug_write("find better score:"+ str(best_score))

        #gamelib.debug_write("Best demolisher score: " + str(d_best_score))
        self.print_stats(best_location, unit_type, best_all)

        if score_th is None or best_score > score_th:
            game_state.attempt_spawn(unit_type, best_location, best_number)
