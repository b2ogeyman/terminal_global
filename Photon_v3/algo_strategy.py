...
class AlgoStrategy(gamelib.AlgoCore):
        ...
        for x in range(28):
            for y in range(14):
                ...
                if unit and unit.health < unit.max_health * .70:
                    ...
                    self.marked_repair_deletion.append([[x,y], unit.unit_type])

        for item in self.marked_repair_deletion:
            loc, unit_type = item[0], item[1]
            unit = game_state.contains_stationary_unit(loc)
            if not unit:
                if game_state.attempt_spawn(unit_type, loc) == 1:
                    self.marked_repair_deletion.remove(item)
    ...
    def shoot_your_shot(self, game_state):
        ...
        '''
        if game_state.get_resource(0) > 6 and random.random()>0.6:
