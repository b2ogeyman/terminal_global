...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_turn(self, turn_state):
        ...
        if game_state.turn_number == 25 and not self.used_scout and self.used_demolisher:
            if self.left_dem_crossing > self.right_dem_crossing:
                ...
                self.turrets += [[2, 12], [1, 12]]
                self.buff_walls += [[2, 13], [1, 13]]
                self.upgrade_buff += [[2, 13], [1, 13]]
                ...
                self.turrets += [[25, 12], [26, 12]]
                self.buff_walls += [[25, 13], [26, 13]]
                self.upgrade_buff += [[25, 13], [26, 13]]
    ...
    def build_defense(self, game_state):
        ...
        if game_state.turn_number % 3 == 1:
            if self.right_buffed:
                game_state.attempt_spawn(WALL, [[4, 10], [5, 10]])
                game_state.attempt_remove([[4, 10], [5, 10]])
            elif self.left_buffed:
                game_state.attempt_spawn(WALL, [[23, 10], [22, 10]])
                game_state.attempt_remove([[23, 10], [22, 10]])
