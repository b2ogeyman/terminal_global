...
class GameState:
    ...
    def find_path_to_edge(self, start_location, target_edge=None):
        ...
        if self.contains_stationary_unit(start_location):
            debug_write("Attempted to perform pathing from blocked starting location {}".format(start_location))
            return
    ...
    def attack_score(self, deploy_location, deploy_number, unit_type):
        ...
        for loc in support_band:
            if self.contains_stationary_unit(loc) and self.game_map[loc][0].unit_type == SUPPORT:
    ...
    #        debug_write('Unit_health %d, deploy_number %d ' % (unit_health, number_left))
        ...
        while len(path) > 0:
            ...
            for attacker_location in path:
                    ...
                    for j in range(number_left):
                        ...
                        if best_unit:
                            ...
                            if  best_unit.health == damage:
                                ...
                                killed = True
                                score += best_unit.cost[0] * 20
                                kill_score += best_unit.cost[0] * 20
                            else:
                ...
                if killed:
                    new_path = self.find_path_to_edge(attacker_location, target_edge)
                    break
                
                if attacker_location == path[-1]:
                    opp_edges = self.game_map.get_edge_locations(self.game_map.TOP_LEFT) + self.game_map.get_edge_locations(self.game_map.TOP_RIGHT)
                    if attacker_location in opp_edges:
        ...
        return {'score' : score, 'dam_score' : dam_score, 'kill_score' : kill_score, 'edge_score' : edge_score, 'total_path' : total_path} 
